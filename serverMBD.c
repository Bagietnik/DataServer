...
#define TIMEOUT 60 
#define LOG_FILE "server_log.txt"  // Plik logów
...
int current_number2 = 84;

void log_connection(const char *client_ip, int client_port) {
...
void handle_client(SSL *ssl) {
    ...
    if (strncmp(buffer, "GET", 3) == 0) {
        ...
        snprintf(json_response, sizeof(json_response), "{\"number1\": %d, \"number2\": %d}", current_number, current_number2);
        ...
        if (body != NULL) {
            ...
            int new_number1, new_number2;
            int parsed_values = sscanf(body, "{\"number1\": %d, \"number2\": %d}", &new_number1, &new_number2);
            if (parsed_values == 2) {
                current_number = new_number1;
                current_number2 = new_number2;
                send_response(ssl, "200 OK", "application/json", "{\"status\": \"Numbers updated\"}");
            } else {
                send_response(ssl, "400 Bad Request", "application/json", "{\"error\": \"Two values (number1 and number2) are required\"}");
            }
        } else {
...
void *server_thread(void *arg) {
    ...
    while (1) {
        ...
        // Zalogowanie połączenia
        log_connection(inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
...
int main() {
    ...
    const SSL_METHOD *method = TLS_server_method();  // Obsługuje TLS 1.3
